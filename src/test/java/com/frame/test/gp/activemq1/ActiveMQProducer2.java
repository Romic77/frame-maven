...
public class ActiveMQProducer2 {
	...
	public static void main(String[] args) throws JMSException {
		...
		//6.设置持久化/费持久化特性 如果费持久化，那么意味着MQ的重启会导致消息丢失
		...
		mapMessage.setStringProperty("name","zhangsan");
		...
		//最后两个参数 一个是priority，消息优先级，最后一个是TTL,消息存活时间
		messageProducer.send(destination,mapMessage,DeliveryMode.NON_PERSISTENT,4,1000*60*10);
		messageProducer.send(destination,mapMessage2,DeliveryMode.NON_PERSISTENT,4,1000*60*10);
		messageProducer.send(destination,mapMessage3,DeliveryMode.NON_PERSISTENT,4,1000*60*10);
